(() => {
    ...
    const cityButton = document.querySelector(".header__cities-button");
    if (cityButton) {
        const cityBody = document.querySelector(".header__cities-body");
        const cityItems = document.querySelectorAll(".header__cities-item");
        cityButton.addEventListener("click", (function(e) {
            e.preventDefault();
            cityBody.classList.toggle("_active");
        }));
        for (let i = 0; i < cityItems.length; i++) cityItems[i].addEventListener("click", (function() {
            cityBody.classList.remove("_active");
        }));
        document.addEventListener("click", (function(e) {
            const target = e.target;
            const its_cityBody = target == cityBody || cityBody.contains(target);
            const its_cityButton = target == cityButton;
            const cityBody_is_active = cityBody.classList.contains("_active");
            if (!its_cityBody && !its_cityButton && cityBody_is_active) cityBody.classList.remove("_active");
        }));
    }
    const subtitle = document.querySelector(".subtitle");
    if (subtitle) {
        const subtitleBody = document.querySelector(".header__subtitle");
        subtitle.addEventListener("click", (function(e) {
            e.preventDefault();
            subtitleBody.classList.toggle("_active");
            subtitle.classList.toggle("_active");
        }));
        document.addEventListener("click", (function(e) {
            const target = e.target;
            const its_subtitleBody = target == subtitleBody || subtitleBody.contains(target);
            const its_subtitle = target == subtitle;
            const subtitleBody_is_active = subtitleBody.classList.contains("_active");
            if (!its_subtitleBody && !its_subtitle && subtitleBody_is_active) {
                subtitleBody.classList.remove("_active");
